...
public:
	...
	static TArray<FFormatterEdge*> GetEdgeBetweenTwoLayer(
		const TArray<FFormatterNode*>& LowerLayer,
		const TArray<FFormatterNode*>& UpperLayer,
		const FFormatterNode* ExcludedNode = nullptr
	);
	...
	// Stable spatial index assigned whenever this node is placed in a layer.
	int32 IndexInLayer{ -1 };
...
{
public:
	static TArray<FBox2D> CalculateLayersBound(
		TArray<TArray<FFormatterNode*>>& InLayeredNodes, bool IsHorizontalDirection = true, bool IsParameterGroup = false
	);
	...
	// Retained for settings and API compatibility. A topology rank is no longer
	// split into artificial ranks merely to satisfy this visual width hint.
...
public:
	explicit FDisconnectedGraph(bool InIsVerticalLayout = false, bool InIsParameterGroup = false);

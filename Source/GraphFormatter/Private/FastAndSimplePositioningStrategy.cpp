...
TArray<FFormatterNode*> FFastAndSimplePositioningStrategy::GetSortedAdjacencies(FFormatterNode* Node, bool bUppers) const
{
	TArray<FFormatterNode*> Result = bUppers ? Node->GetUppers() : Node->GetLowers();
	Result.Sort(
		[this](const FFormatterNode& A, const FFormatterNode& B)
		{
			return PosMap.FindChecked(const_cast<FFormatterNode*>(&A))
				 < PosMap.FindChecked(const_cast<FFormatterNode*>(&B));
		}
	);
	return Result;
}
...
void FFastAndSimplePositioningStrategy::Initialize()
{
	for (auto& Layer : LayeredNodes)
	{
		for (int32 i = 0; i < Layer.Num(); i++)
		{
			...
			if (i != 0) { PredecessorMap.Add(Layer[i], Layer[i - 1]); }
			else
			{
				PredecessorMap.Add(Layer[i], nullptr);
			}
			if (i != Layer.Num() - 1) { SuccessorMap.Add(Layer[i], Layer[i + 1]); }
			else
			{
	...
	for (int32 i = 1; i < LayeredNodes.Num() - 1; i++)
	{
		...
		for (int32 l1 = 0; l1 < LayeredNodes[i + 1].Num(); l1++)
		{
			...
			if (l1 == LayeredNodes[i + 1].Num() - 1 || IsCrossingInnerSegment)
			{
				...
				{
					const TArray<FFormatterNode*> Uppers = GetSortedAdjacencies(Node, true);
					if (Uppers.Num() > 0) { k1 = PosMap.FindChecked(Uppers[Uppers.Num() / 2]); }
				}
				while (l <= l1)
				{
					FFormatterNode* LowerNode = LayeredNodes[i + 1][l];
					const TArray<FFormatterNode*> UpperNodes = GetSortedAdjacencies(LowerNode, true);
					for (auto UpperNode : UpperNodes)
					{
						const int32 k = PosMap.FindChecked(UpperNode);
						if (k < k0 || k > k1) { ConflictMarks.AddUnique(UpperNode, LowerNode); }
					}
					++l;
				}
				k0 = k1;
...
void FFastAndSimplePositioningStrategy::DoVerticalAlignment()
{
	...
	for (int32 i = LayerStart; i != LayerEnd; i += LayerStep)
	{
		...
		for (int32 k = Start; k != End; k += Step)
		{
			...
			auto Adjacencies = GetSortedAdjacencies(Node, IsUpperDirection);
			if (Adjacencies.Num() > 0)
			{
				...
				for (int32 m = ma; m <= mb; m++)
				{
					if (AlignMap[Node] == Node)
					{
						...
						bool IsMarked = ConflictMarks.FindPair(MedianNode, Node) != nullptr;
						float MaxWeight = MedianNode->GetMaxWeight(IsUpperDirection ? EGPD_Output : EGPD_Input);
						float LinkWeight =
							Node->GetMaxWeightToNode(MedianNode, IsUpperDirection ? EGPD_Input : EGPD_Output);
...
void FFastAndSimplePositioningStrategy::DoHorizontalCompaction()
{
	...
	for (auto& Layer : LayeredNodes)
	{
		for (auto Node : Layer)
		{
			...
			if (IsLeftDirection) { ShiftMap.Add(Node, FLT_MAX); }
			else
			{
	...
	for (auto& Layer : LayeredNodes)
	{
		for (auto Node : Layer)
		{
			if (RootMap[Node] == Node) { PlaceBlock(Node); }
		}
	}
	for (auto& Layer : LayeredNodes)
	{
		...
		do
		{
			...
			if (Adjacency != nullptr)
			{
				...
				if (IsHorizontalDirection)
				{
					...
					if (IsParameterGroup) { Spacing *= Settings.SpacingFactorOfParameterGroup.Y; }
				}
				else
				{
					...
					if (IsParameterGroup) { Spacing *= Settings.SpacingFactorOfParameterGroup.X; }
				}

				const auto PrevBlockRoot = RootMap[Adjacency];
				PlaceBlock(PrevBlockRoot);
				if (SinkMap[BlockRoot] == BlockRoot) { SinkMap[BlockRoot] = SinkMap[PrevBlockRoot]; }
				if (SinkMap[BlockRoot] != SinkMap[PrevBlockRoot])
				{
					float LeftShift = (*XMap)[BlockRoot] - (*XMap)[PrevBlockRoot] + InnerShiftMap[Node]
									- InnerShiftMap[Adjacency] - AdjacencyHeight - Spacing;
					float RightShift = (*XMap)[BlockRoot] - (*XMap)[PrevBlockRoot] - InnerShiftMap[Node]
									 + InnerShiftMap[Adjacency] + NodeHeight + Spacing;
					float Shift = IsLeftDirection ? FMath::Min(ShiftMap[SinkMap[PrevBlockRoot]], LeftShift)
												  : FMath::Max(ShiftMap[SinkMap[PrevBlockRoot]], RightShift);
				...
				else
				{
					...
					{
						Position = IsLeftDirection ? FMath::Max((*XMap)[BlockRoot], Position)
												   : FMath::Min((*XMap)[BlockRoot], Position);
						(*XMap)[BlockRoot] = Position;
					}
				}
			}
	...
	for (auto& Layer : LayeredNodes)
	{
		for (auto Node : Layer)
		{
			if (RootMap[Node] == Node)
			{
				...
				while (true)
				{
					const float UpperPosition = UpperNode->GetLinkedPositionToNode(
						LowerNode, IsUpperDirection ? EGPD_Output : EGPD_Input, IsHorizontalDirection
					);
					const float LowerPosition = LowerNode->GetLinkedPositionToNode(
						UpperNode, IsUpperDirection ? EGPD_Input : EGPD_Output, IsHorizontalDirection
					);
...
{
	TArray<TMap<FFormatterNode*, float>> Layouts = {
		UpperLeftPositionMap, UpperRightPositionMap, LowerLeftPositionMap, LowerRightPositionMap
	};
	...
	for (int32 i = 0; i < Layouts.Num(); i++)
	{
		...
		{
			if (Pair.Value < LeftMost) { LeftMost = Pair.Value; }
			if (Pair.Value > RightMost) { RightMost = Pair.Value; }
		}
	...
	for (auto& Layer : LayeredNodes)
	{
		for (auto Node : Layer)
		{
			...
			Values.Sort();
			if (!IsHorizontalDirection) { CombinedPositionMap.Add(Node, (Values[1] + Values[2]) / 2.0f); }
			else
			{
...
FFastAndSimplePositioningStrategy::FFastAndSimplePositioningStrategy(
	...
	, IsParameterGroup(IsParameterGroup)
{
	...
	for (int i = 0; i < LayeredNodes.Num(); i++)
	{
		...
		for (auto Node : Layer)
		{
			...
			{
				if (IsHorizontalDirection) { *pX = LayersBound[i].Max.X - Node->Size.X; }
				else
				{
			...
			{
				if (IsHorizontalDirection) { *pX = LayersBound[i].Min.X; }
				else
				{
	...
	for (int32 i = 0; i < InLayeredNodes.Num(); i++)
	{
		for (auto Node : InLayeredNodes[i])
		{
			...
			if (Bound.bIsValid) { Bound += FBox2D(Node->GetPosition(), Node->GetPosition() + Node->Size); }
			else
			{

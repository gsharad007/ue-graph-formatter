...
namespace
{
bool IsFormatterGuidBefore(const FGuid& A, const FGuid& B)
{
...
int32 GetNodeOrderIndex(const FFormatterNode& Node) { return Node.IndexInLayer; }

bool IsNodeBeforeInLayer(const FFormatterNode& A, const FFormatterNode& B)
{
	const int32 AIndex = GetNodeOrderIndex(A);
	const int32 BIndex = GetNodeOrderIndex(B);
	if (AIndex != BIndex) { return AIndex < BIndex; }
	return IsFormatterGuidBefore(A.Guid, B.Guid);
}
} // namespace
...
{
	if (Other.SubGraph != nullptr) { SubGraph = Other.SubGraph->Clone(); }
	else
	{
		SubGraph = nullptr;
	}
...
void FFormatterNode::Connect(FFormatterPin* SourcePin, FFormatterPin* TargetPin, float Weight)
{
	...
	Edge->To = TargetPin;
	Edge->Weight = Weight;
	if (SourcePin->Direction == EFormatterPinDirection::Out) { OutEdges.Add(Edge); }
	else
	{
		InEdges.Add(Edge);
	}
...
{
	if (Pin->Direction == EFormatterPinDirection::In) { InPins.Add(Pin); }
	else
	{
		OutPins.Add(Pin);
	}
...
bool FFormatterNode::IsSource() const { return InEdges.Num() == 0; }

bool FFormatterNode::IsSink() const { return OutEdges.Num() == 0; }

bool FFormatterNode::AnySuccessorPathDepthEqu0() const
{
	for (auto OutEdge : OutEdges)
	{
		if (OutEdge->To->OwningNode->PathDepth == 0) { return true; }
	}
	return false;
}
...
float FFormatterNode::GetLinkedPositionToNode(const FFormatterNode* Node, EEdGraphPinDirection Direction, bool IsHorizontalDirection)
{
	...
	for (auto Edge : Edges)
	{
		...
		{
			if (IsHorizontalDirection) { MedianPosition += Edge->From->NodeOffset.Y; }
			else
			{
...
float FFormatterNode::GetMaxWeight(EEdGraphPinDirection Direction)
{
	...
	for (auto Edge : Edges)
	{
		if (MaxWeight < Edge->Weight) { MaxWeight = Edge->Weight; }
	}
	return MaxWeight;
}
...
float FFormatterNode::GetMaxWeightToNode(const FFormatterNode* Node, EEdGraphPinDirection Direction)
{
	...
	for (auto Edge : Edges)
	{
		if (Edge->To->OwningNode == Node && MaxWeight < Edge->Weight) { MaxWeight = Edge->Weight; }
	}
	return MaxWeight;
}
...
{
	const TArray<FFormatterNode*> UpperNodes = GetUppers();
	if (UpperNodes.Num() > 0)
	{
...
{
	const TArray<FFormatterNode*> LowerNodes = GetLowers();
	if (LowerNodes.Num() > 0)
	{
...
TArray<FFormatterNode*> FFormatterNode::GetUppers() const
{
	...
	{
		UpperNodes.AddUnique(InEdge->To->OwningNode);
	}
	UpperNodes.Sort([](const FFormatterNode& A, const FFormatterNode& B) { return IsNodeBeforeInLayer(A, B); });
	return UpperNodes;
}
...
TArray<FFormatterNode*> FFormatterNode::GetLowers() const
{
	...
	for (auto OutEdge : OutEdges)
	{
		LowerNodes.AddUnique(OutEdge->To->OwningNode);
	}
	LowerNodes.Sort([](const FFormatterNode& A, const FFormatterNode& B) { return IsNodeBeforeInLayer(A, B); });
	return LowerNodes;
}

int32 FFormatterNode::GetInputPinCount() const { return InPins.Num(); }

int32 FFormatterNode::GetInputPinIndex(FFormatterPin* InputPin) const { return InPins.Find(InputPin); }

int32 FFormatterNode::GetOutputPinCount() const { return OutPins.Num(); }

int32 FFormatterNode::GetOutputPinIndex(FFormatterPin* OutputPin) const { return OutPins.Find(OutputPin); }
...
TArray<FFormatterEdge*> FFormatterNode::GetEdgeLinkedToLayer(const TArray<FFormatterNode*>& Layer, EEdGraphPinDirection Direction) const
{
	...
	for (auto Edge : Edges)
	{
		...
		{
			if (Edge->To->OwningNode == NextLayerNode) { Result.Add(Edge); }
		}
	}
	return Result;
}
...
{
	if (OriginalNode == nullptr) { return 0; }
...
void FFormatterNode::InitPosition(FVector2D InPosition) { Position = InPosition; }

void FFormatterNode::SetPosition(FVector2D InPosition)
{
	const FVector2D Offset = InPosition - Position;
	Position = InPosition;
	if (SubGraph != nullptr) { SubGraph->OffsetBy(Offset); }
}

FVector2D FFormatterNode::GetPosition() const { return Position; }
...
void FFormatterNode::UpdatePinsOffset(FVector2D Border)
{
	if (SubGraph != nullptr)
	{
		...
		for (auto Pin : InPins)
		{
			if (PinsOffset.Contains(Pin->OriginalPin)) { Pin->NodeOffset = PinsOffset[Pin->OriginalPin] + Border; }
		}
		for (auto Pin : OutPins)
		{
			if (PinsOffset.Contains(Pin->OriginalPin)) { Pin->NodeOffset = PinsOffset[Pin->OriginalPin] + Border; }
		}
		InPins.Sort([](const FFormatterPin& A, const FFormatterPin& B) { return A.NodeOffset.Y < B.NodeOffset.Y; });
		OutPins.Sort([](const FFormatterPin& A, const FFormatterPin& B) { return A.NodeOffset.Y < B.NodeOffset.Y; });
	}
}
...
TArray<FFormatterNode*> FFormatterNode::GetSuccessorsForNodes(TSet<FFormatterNode*> Nodes)
{
	...
	for (auto Node : Nodes)
	{
		...
		{
			if (!Nodes.Contains(OutEdge->To->OwningNode)) { Result.Add(OutEdge->To->OwningNode); }
		}
	}
	return Result;
}
...
TArray<FFormatterNode*> FConnectedGraph::GetNodesGreaterThan(int32 i, TSet<FFormatterNode*>& Excluded)
{
	...
	for (auto Node : Nodes)
	{
		if (!Excluded.Contains(Node) && Node->PathDepth >= i) { Result.Add(Node); }
	}
	return Result;
}
...
TArray<FBox2D> FFormatterGraph::CalculateLayersBound(
	TArray<TArray<FFormatterNode*>>& InLayeredNodes, bool IsHorizontalDirection, bool IsParameterGroup
)
{
	TArray<FBox2D> LayersBound;
	FBox2D TotalBound(ForceInit);
	FVector2D Spacing;
	if (IsHorizontalDirection) { Spacing = FVector2D(HorizontalSpacing, 0); }
	else
	{
		Spacing = FVector2D(0, VerticalSpacing);
	}
	if (IsParameterGroup) { Spacing *= SpacingFactorOfGroup.X; }
	for (int32 i = 0; i < InLayeredNodes.Num(); i++)
	{
		const auto& Layer = InLayeredNodes[i];
		FBox2D Bound(ForceInit);
		FVector2D Position;
		if (TotalBound.bIsValid) { Position = TotalBound.Max + Spacing; }
		else
		{
		...
		for (auto Node : Layer)
		{
			if (Bound.bIsValid) { Bound += FBox2D(Position, Position + Node->Size); }
			else
			{
		...
		if (TotalBound.bIsValid) { TotalBound += Bound; }
		else
		{
			TotalBound = Bound;
		}
...
void FFormatterGraph::AddNode(FFormatterNode* InNode)
{
	...
	if (InNode->SubGraph != nullptr) { SubGraphs.Add(InNode->Guid, InNode->SubGraph); }
	for (auto Pin : InNode->InPins)
	{
		if (Pin->OriginalPin != nullptr) { OriginalPinsMap.Add(Pin->OriginalPin, Pin); }
...
FBox2D FFormatterGraph::GetBorder() const { return Border; }

void FFormatterGraph::SetIsParameterGroup(bool InIsParameterGroup) { IsParameterGroup = InIsParameterGroup; }

bool FFormatterGraph::GetIsParameterGroup() const { return IsParameterGroup; }

bool FFormatterGraph::GetIsVerticalLayout() const { return IsVerticalLayout; }

void FFormatterGraph::SetIsVerticalLayout(bool InIsVerticalLayout) { IsVerticalLayout = InIsVerticalLayout; }
...
FFormatterGraph* FFormatterGraph::Clone() { return nullptr; }

void FDisconnectedGraph::AddGraph(FFormatterGraph* Graph) { ConnectedGraphs.Push(Graph); }
...
FDisconnectedGraph::FDisconnectedGraph(bool InIsVerticalLayout, bool InIsParameterGroup)
...
FFormatterGraph* FConnectedGraph::Clone() { return new FConnectedGraph(*this); }
...
TArray<FFormatterNode*> FConnectedGraph::GetLeavesWithPathDepth0() const
{
	...
	for (auto Node : Nodes)
	{
		if (Node->PathDepth != 0 || Node->AnySuccessorPathDepthEqu0()) { continue; }
		Result.Add(Node);
	}
	return Result;
}
...
void FConnectedGraph::DoLayering()
{
	...
	for (int32 i = AssignPathDepthForNodes(); i != 0; i--)
	{
		...
		for (auto Node : NodesToProcess)
		{
			...
			if (bPredecessorsFinished) { Layer.Add(Node); }
		}
		Set.Append(Layer);
		TArray<FFormatterNode*> Array = Layer.Array();
		Array.Sort(
			[this](const FFormatterNode& A, const FFormatterNode& B)
			{
				if (NodeComparer)
				{
					const bool bAB = NodeComparer(A, B);
					const bool bBA = NodeComparer(B, A);
					if (bAB != bBA) { return bAB; }
				}

				const FVector2D APosition = A.GetPosition();
				const FVector2D BPosition = B.GetPosition();
				const double APrimary = IsVerticalLayout ? APosition.X : APosition.Y;
				const double BPrimary = IsVerticalLayout ? BPosition.X : BPosition.Y;
				if (APrimary != BPrimary) { return APrimary < BPrimary; }
				const double ASecondary = IsVerticalLayout ? APosition.Y : APosition.X;
				const double BSecondary = IsVerticalLayout ? BPosition.Y : BPosition.X;
				if (ASecondary != BSecondary) { return ASecondary < BSecondary; }

				// Nodes is deterministically seeded by the adapter (or by caller
				// insertion order for the runtime graph API), so this is a stable,
				// total tie-break even for nodes without persistent GUIDs.
				return Nodes.Find(const_cast<FFormatterNode*>(&A)) < Nodes.Find(const_cast<FFormatterNode*>(&B));
			}
		);

		// MaxLayerNodes remains in the settings/API for compatibility. Splitting
		// a real topology rank into synthetic ranks changes edge direction and
		// made membership depend on TSet iteration order, so a rank now stays whole.
		LayeredList.Add(MoveTemp(Array));
	}
}
...
void FConnectedGraph::AddDummyNodes()
{
	for (int i = 0; i < LayeredList.Num() - 1; i++)
	{
		...
		for (auto Node : Layer)
		{
			...
			for (auto Edge : Node->OutEdges)
			{
				if (!NextLayer.Contains(Edge->To->OwningNode)) { LongEdges.Add(Edge); }
			}
...
void FConnectedGraph::SortInLayer(TArray<TArray<FFormatterNode*>>& Order, EEdGraphPinDirection Direction)
{
	...
	for (int i = Start; i != End; i += Step)
	{
		...
		TMap<FFormatterNode*, int32> PreviousOrder;
		for (int32 NodeIndex = 0; NodeIndex < FreeLayer.Num(); ++NodeIndex)
		{
			PreviousOrder.Add(FreeLayer[NodeIndex], NodeIndex);
		}
		FreeLayer.Sort(
			[&PreviousOrder](const FFormatterNode& A, const FFormatterNode& B) -> bool
			{
				if (A.OrderValue != B.OrderValue) { return A.OrderValue < B.OrderValue; }
				return PreviousOrder.FindChecked(const_cast<FFormatterNode*>(&A))
					 < PreviousOrder.FindChecked(const_cast<FFormatterNode*>(&B));
			}
		);
...
TArray<FFormatterEdge*> FFormatterNode::GetEdgeBetweenTwoLayer(
	const TArray<FFormatterNode*>& LowerLayer, const TArray<FFormatterNode*>& UpperLayer, const FFormatterNode* ExcludedNode
)
{
	TArray<FFormatterEdge*> Result;
	for (auto Node : LowerLayer)
	{
		if (ExcludedNode == Node) { continue; }
...
int32 FFormatterNode::CalculateCrossing(const TArray<TArray<FFormatterNode*>>& Order)
{
	...
	for (int i = 1; i < Order.Num(); i++)
	{
		...
		while (NodeEdges.Num() != 0)
		{
			...
			{
				if (Edge1->IsCrossing(Edge2)) { CrossingValue++; }
			}
		}
	}
...
void FConnectedGraph::DoOrderingSweep()
{
	...
	int32 BestCrossing = FFormatterNode::CalculateCrossing(Best);
	...
	LayeredList = MoveTemp(Best);
	FFormatterNode::CalculatePinsIndex(LayeredList);
}
...
TSet<void*> FConnectedGraph::GetOriginalNodes() const
{
	...
	for (auto Node : Nodes)
	{
		if (SubGraphs.Contains(Node->Guid)) { Result.Append(SubGraphs[Node->Guid]->GetOriginalNodes()); }
		if (Node->OriginalNode != nullptr) { Result.Add(Node->OriginalNode); }
	}
	return Result;
}
...
void FConnectedGraph::Format()
{
	for (auto [Key, SubGraph] : SubGraphs)
	{
		...
		Node->Size = SubGraph->GetTotalBound().GetSize()
				   + FVector2D(SubGraphBorder.Min.X + SubGraphBorder.Max.X, SubGraphBorder.Min.Y + SubGraphBorder.Max.Y);
	}
	if (Nodes.Num() > 0)
	{
		RemoveCycle();
		DoLayering();
		AddDummyNodes();
		if (!NodeComparer) { DoOrderingSweep(); }
		FFormatterNode::CalculatePinsIndex(LayeredList);
		DoPositioning();
	}
}
...
TMap<void*, FBox2D> FConnectedGraph::GetBoundMap()
{
	...
	for (auto Node : Nodes)
	{
		if (Node->OriginalNode == nullptr) { continue; }
		Result.Add(Node->OriginalNode, FBox2D(Node->GetPosition(), Node->GetPosition() + Node->Size));
		if (SubGraphs.Contains(Node->Guid)) { Result.Append(SubGraphs[Node->Guid]->GetBoundMap()); }
	}
	return Result;
}
